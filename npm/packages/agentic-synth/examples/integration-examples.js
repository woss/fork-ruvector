...
exports.expressAPIExample = expressAPIExample;
exports.prismaSeedingExample = prismaSeedingExample;
exports.jestFixturesExample = jestFixturesExample;
exports.tensorflowTrainingExample = tensorflowTrainingExample;
exports.graphqlMockingExample = graphqlMockingExample;
exports.redisCachingExample = redisCachingExample;
exports.kafkaStreamingExample = kafkaStreamingExample;
exports.elasticsearchIndexingExample = elasticsearchIndexingExample;
exports.nextjsAPIRouteExample = nextjsAPIRouteExample;
exports.supabaseIntegrationExample = supabaseIntegrationExample;
 ...
 */
async function prismaSeedingExample() {
 ...
 */
async function jestFixturesExample() {
    console.log('\n🧪 Example 3: Jest Testing Fixtures\n');
    const synth = new index_js_1.AgenticSynth({
        provider: 'gemini',
        apiKey: process.env.GEMINI_API_KEY || 'demo-key',
    ...
    console.log('- Admin user:', testUsers.data.find((u) => u.role === 'admin'));
    console.log('- Regular user:', testUsers.data.find((u) => u.role === 'user'));
    console.log('- Inactive user:', testUsers.data.find((u) => !u.active));
    // Example test usage
    const exampleTest = () => {
 ...
 */
async function tensorflowTrainingExample() {
    console.log('\n🤖 Example 4: TensorFlow.js Training Data\n');
    const synth = new index_js_1.AgenticSynth({
        provider: 'gemini',
        apiKey: process.env.GEMINI_API_KEY || 'demo-key',
    ...
    const labels = trainingData.data.map((d) => d.label);
    console.log('Training Data Statistics:');
    console.log(`- Total samples: ${features.length}`);
    console.log(`- Positive samples: ${labels.filter((l) => l === 1).length}`);
    console.log(`- Negative samples: ${labels.filter((l) => l === 0).length}`);
 ...
 */
async function graphqlMockingExample() {
    ...
    const resolvers = {
        Query: {
            ...
            user: (_, { id }) => mockUsers.data.find((u) => u.id === id),
        },
        User: {
            posts: (user) => mockPosts.data.filter((p) => p.authorId === user.id),
        },
    };
 ...
 */
async function redisCachingExample() {
    console.log('\n💾 Example 6: Redis Caching Integration\n');
    const synth = new index_js_1.AgenticSynth({
        provider: 'gemini',
        apiKey: process.env.GEMINI_API_KEY || 'demo-key',
    ...
    async function getCachedData(cacheKey, generator) {
        // Check Redis first
 ...
 */
async function kafkaStreamingExample() {
 ...
 */
async function elasticsearchIndexingExample() {
    console.log('\n🔍 Example 8: Elasticsearch Bulk Indexing\n');
    const synth = new index_js_1.AgenticSynth({
        provider: 'gemini',
        apiKey: process.env.GEMINI_API_KEY || 'demo-key',
 ...
 */
async function nextjsAPIRouteExample() {
    console.log('\n⚡ Example 9: Next.js API Route\n');
    const synth = new index_js_1.AgenticSynth({
        provider: 'gemini',
        apiKey: process.env.GEMINI_API_KEY || 'demo-key',
 ...
 */
async function supabaseIntegrationExample() {
    console.log('\n🔥 Example 10: Supabase Integration\n');
    const synth = new index_js_1.AgenticSynth({
        provider: 'gemini',
        apiKey: process.env.GEMINI_API_KEY || 'demo-key',
...
//# sourceMappingURL=integration-examples.js.map