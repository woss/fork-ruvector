...
exports.setupDSPyOptimization = setupDSPyOptimization;
exports.generateWithDSPy = generateWithDSPy;
exports.compareResults = compareResults;
exports.calculateQualityMetrics = calculateQualityMetrics;
exports.calculateAverageQuality = calculateAverageQuality;
exports.createTrainingExamples = createTrainingExamples;
 ...
 */
function calculateQualityMetrics(product) {
    ...
    const completeness = ((hasLength ? 0.4 : 0) +
        (hasFeatures ? 0.3 : 0) +
        (hasCTA ? 0.3 : 0));
 ...
 */
function calculateAverageQuality(products) {
 ...
 */
const productQualityMetric = (0, dspy_ts_1.createMetric)('product-quality', (example, prediction) => {
 ...
 */
async function generateBaseline(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const priceRangeKey = ['low', 'medium', 'high'][Math.floor(Math.random() * 3)];
 ...
 */
async function setupDSPyOptimization() {
    ...
    const lm = new dspy_ts_1.OpenAILM({
    ...
    const optimizer = new dspy_ts_1.BootstrapFewShot({
 ...
 */
async function generateWithDSPy(optimizedModule, count) {
    ...
    const totalTokens = count * avgTokensPerProduct;
    const estimatedCost = (totalTokens / 1000000) * 1.0; // Average of input/output
 ...
 */
function compareResults(baselineData, optimizedData) {
 ...
 */
function exportResults(results, filename = 'dspy-comparison-results.json') {
...
//# sourceMappingURL=dspy-complete-example.js.map