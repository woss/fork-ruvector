...
exports.CodeSkills = exports.CodeGenerateSkill = exports.CodeExplainSkill = void 0;
...
async function* codeExplainExecutor(context, params) {
    ...
    if (language === 'javascript' || language === 'typescript') {
        if (code.includes('async'))
            hints.push('Uses async/await for asynchronous operations');
        if (code.includes('=>'))
            hints.push('Uses arrow functions');
        if (code.includes('class '))
            hints.push('Defines classes (OOP pattern)');
        if (code.includes('export'))
            hints.push('Exports modules (ES6 modules)');
    }
    if (language === 'python') {
        if (code.includes('async def'))
            hints.push('Uses async/await for asynchronous operations');
        if (code.includes('@'))
            hints.push('Uses decorators');
        if (code.includes('class '))
            hints.push('Defines classes (OOP pattern)');
        if (code.includes('with '))
            hints.push('Uses context managers');
    }
    if (language === 'rust') {
        if (code.includes('async fn'))
            hints.push('Uses async functions');
        if (code.includes('impl '))
            hints.push('Implements traits or methods');
        if (code.includes('unsafe '))
            hints.push('Contains unsafe code blocks');
        if (code.includes('Result<'))
            hints.push('Uses Result for error handling');
    }
    if (hints.length > 0) {
...
async function* codeGenerateExecutor(context, params) {
    const description = params.description;
    const language = params.language || 'typescript';
    const style = params.style || 'documented';
    yield {
        type: 'message',
...
exports.CodeSkills = [exports.CodeExplainSkill, exports.CodeGenerateSkill];
exports.default = exports.CodeSkills;
//# sourceMappingURL=CodeSkill.js.map